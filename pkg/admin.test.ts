...
test(
  ...
  async () => {
    ...
    const admin = new Kafka({ url, username: "username", password: "password" })
      .admin();
  ...
  () => {
    test(
      ...
      async () => {
        ...
        expect(
          consumers.find((c) => c.name === consumerGroupId)!.instances.map(
            (i) => i.name,
          ),
...
describe(
  ...
  () => {
    test(
      "returns the latest offsets for one topicPartition",
      async () => {
        const consumerGroupId = randomUUID();
        const instanceId = randomUUID();
        ...
        while (!ready) {
          ...
          const consumer = consumerGroups.find(
            (c) => c.name === consumerGroupId,
          );
          if (consumer) {
            const instance = consumer.instances.find(
              (i) => i.name === instanceId,
            );
            if (instance) {
...
describe(
  ...
  () => {
    test(
      "returns the offsets of a single partition and topic",
      async () => {
        const admin = kafka.admin();
        const p = kafka.producer();

        const { topic, partition } = await p.produce(
          Topic.RED,
          randomUUID(),
          { partition: 0 },
        );
    ...
    test(
      "returns the offsets of multiple partition and topic",
      async () => {
        const admin = kafka.admin();
        const p = kafka.producer();
